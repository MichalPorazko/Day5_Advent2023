...
object ReadingFile {
  ...
  def parseAlmanac(filePath: String): Almanac = 
    ...
    val resources = sections.flatMap { section =>
      ...
      val sectionType = lines.head match {
        case "seed-to-soil map:" => Section.Seed
        case "soil-to-fertilizer map:" => Section.Soil
        case "fertilizer-to-water map:" => Section.Fertilizer
        case "water-to-light map:" => Section.Water
        case "light-to-temperature map:" => Section.Light
        case "temperature-to-humidity map:" => Section.Temperature
        case "humidity-to-location map:" => Section.Humidity
      }
  ...
  def splittingToResources(s: Array[String], section: Section): List[Resource] =
    s.tail.map { entry =>
        ...
        Resource(RangeFunction(destination, source, length), section)
      }.toList
